...
install_uv() {
    ...
    [[ -s "$installer" ]] || fail "The downloaded uv installer is empty: $installer"
...
validate_uv() {
    ...
    if ! uv --version >/dev/null 2>&1; then
        fail "uv was found at '$uv_path' but could not run successfully."
    fi
...
ensure_python() {
    ...
    if ! uv python install; then
        fail "Python installation failed for the request in .python-version ('$python_request'). Check network access, uv output above, available disk space, and whether this Python build exists for the current platform."
    fi
